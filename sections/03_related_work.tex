...
Por fim, o trabalho discutido em~\cite{Kim2016} mostra uma abordagem sobre STT-RAM voláteis oposta ao dos trabalhos anteriores. Isto porque o artigo defende que a utilização de métodos de \textit{refresh} em STT-RAM voláteis não são confiáveis o suficiente para serem utilizados. Afirma-se que o tempo de retenção da STT-RAM volátil não é determinístico, tampouco pode ser previsto. Além disto, falhas na retenção da STT-RAM volátil não podem ser previstos por causa da natureza estocástica da falha. Com isto, sugere-se a utilização de ECC (\textit{Error Correction Code}) e \textit{Scrubbing} para manter os dados em memória confiáveis. Por fim, uma célula STT-RAM volátil deve ser projetada de maneira cuidadosa no que diz respeito à sua estabilidade termal -- para isso, deve-se levar em consideração: (i) o tamanho do \textit{array} de memória, (ii) a força do código de ECC, (iii) o período de \textit{scrubbing} e (iv) a confiabilidade-alvo.

A relação entre a estabiliadde termal e os quatro itens citados anteriormente é estudada neste trabalho. Alguns resultados são mostrados no sentido da relação entre estes fatores, e.g. com o aumento do período de \textit{scrubbing} de maneira exponencial, o mínimo de estabilidade termal aumenta lenta e linearmente. Além disto, ao usar ECC, reduz-se o mínimo de estabilidade termal comparada a STT-RAM não volátil. Outras observações vistas foram:

\begin{itemize}
	\item Quanto mais frequente o \textit{scrubbing}, mais ineficiente ele tende a ser, dado que o mesmo diminui pouco a estabilidade termal necessária enquanto que o \textit{overhead} gerado pelo \textit{scrubbing} aumenta significativamente em termos de desempnho e energia;
	\item Técnicas de ECC mais simples são melhores para redução da estabilidade termal;
	\item A medida que diminui a taxa de falha exponencialmente, a estabilidade termal também aumenta lentamente de maneira linear.
\end{itemize}

Outros resultados são exibidos verificando o desempenho e a eficiência energética ao usar STT-RAM volátil com \textit{scrubbing} e ECC. A STT-RAM é empregada como LLC, e são testadas 9 diferentes configurações envolvendo o tempo de retenção da memória e diferentes níveis de ECC. Chega-se a conclusão de que a utilização de tanto ECC como \textit{scrubbing} faz com que a STT-RAM volátil torne-se ineficiente em termos de desempenho e energia de maneira geral. Somente o uso da técnica mais simples de ECC com um período de \textit{scrubbing} maior que $10ms$ consegue ter redução de consumo energético comparada à \textit{baseline} (STT-RAM não volátil).